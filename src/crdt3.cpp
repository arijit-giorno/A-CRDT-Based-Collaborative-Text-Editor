// SyncText (Lock-Free) — Part 1–3: registry+monitor+MQ broadcast + Per-line LWW CRDT
...
vector<CRDTOperation> find_changes(const string& user_id,
    ...
    for (size_t ln = 0; ln < max_len; ++ln) {
        ...
        // We still compute ranges for display, but LWW merge will ignore them.
        size_t start = 0;
        while (start < oldL.size() && start < newL.size() && oldL[start] == newL[start]) ++start;
        ...
        if (removed.empty() && !added.empty()) opType = "INSERT";
        else if (!removed.empty() && added.empty()) opType = "DELETE";
        ...
        // Carry full line state for LWW correctness
        // (newContent already contains the delta; we want the complete line too)
        // We’ll overwrite at merge time, so ensure newContent holds full line:
        // Use a sentinel: pack full line into newContent if opType != DELETE
        if (true) {
            op.type = "REPLACE";       // normalize to REPLACE for LWW
            op.startCol = 0;
            op.endCol   = (int)oldL.size();
            op.oldContent = oldL;
            op.newContent = newL;      // FULL LINE CONTENT
        }
...
static void apply_operation_to_line(string& line, const CRDTOperation& op) {
    // Range-based patch (kept for completeness/UI)
...
// NEW: LWW applies whole-line state from the winning op
static void apply_LWW_line(string& line, const CRDTOperation& op) {
    (void)op; // only newContent matters
    line = op.newContent; // overwrite entire line
}

static inline bool newer_than(const CRDTOperation& a, const CRDTOperation& b) {
    if (a.timestamp != b.timestamp) return a.timestamp > b.timestamp;
    // Stable tie-breaker: lexicographically smaller userId wins if timestamps equal
    return a.userId < b.userId;
}

// ---- Per-line LWW merge across local_since_merge ∪ pending_remote ----
static void merge_per_line_LWW(vector<string>& base_lines,
                               const vector<CRDTOperation>& local_since_merge,
                               const vector<CRDTOperation>& remote_ops)
{
    // Combine all ops since the previous merge
    vector<CRDTOperation> all = local_since_merge;
    all.insert(all.end(), remote_ops.begin(), remote_ops.end());
    if (all.empty()) return;

    // Find latest op per line
    map<int, CRDTOperation> winner_per_line;
    for (const auto& op : all) {
        int ln = op.lineNumber;
        auto it = winner_per_line.find(ln);
        if (it == winner_per_line.end() || newer_than(op, it->second)) {
            winner_per_line[ln] = op;
        }
    }

    // Apply winners — OVERWRITE ENTIRE LINE (fixes the “prepend/append” issue)
    for (auto& [ln, win] : winner_per_line) {
        if (ln >= (int)base_lines.size()) base_lines.resize(ln + 1);
        apply_LWW_line(base_lines[ln], win);
    }
}
...
void listener_thread(const string& user_id) {
    ...
    while (running.load(memory_order_relaxed)) {
        ...
        if (n >= 0) {
            ...
            // drop-oldest-on-full to stay lock-free
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        // IMPORTANT: Do NOT auto-reset local file to initial anymore.
        // synchronize_initial_state(user_doc_path);  // <-- removed on purpose
    ...
    while (running.load(memory_order_relaxed)) {
        try {
            ...
            if (need_merge) {
                ...
                // Per-line LWW over ALL ops since previous merge
                ...
                merge_per_line_LWW(new_lines, local_since_merge, pending_remote);
