// SyncText (Lock-Free) — Part 1–3: registry+monitor+MQ broadcast+LWW CRDT
...
// ----------------- Config -----------------
const string REGISTRY_FILE = "registry.json"; // (file-based registry, simple & robust)
...
const size_t MERGE_THRESHOLD = 5;            // merge after (local+remote) >= 5
const int MERGE_MAX_MS = 4000;               // or every 4s if anything pending
const size_t REMOTE_BROADCAST_THRESHOLD = 5; // broadcast remote updates after 5 received
...
vector<string> discover_users()
{
    ...
    for (auto &[uid, ui] : reg)
    {
        if (t - ui.lastSeen < (MONITOR_INTERVAL_MS / 1000.0) * 5.0)
            v.push_back(uid);
    }
    sort(v.begin(), v.end());
    return v;
...
CRDTOperation json_to_op(const string &s)
{
    ...
    auto expect = [&](char ch)
    { skip(); if (i<s.size() && s[i]==ch){++i; return true;} return false; };
    expect('{');
    while (i < s.size())
    {
        skip();
        ...
        if (key == "type" || key == "userId" || key == "oldContent" || key == "newContent")
        {
            if (s[i] != '"')
                break;
            string val;
            read_json_string(s, i, val);
            if (key == "type")
                op.type = val;
            else if (key == "userId")
                op.userId = val;
            else if (key == "oldContent")
                op.oldContent = val;
            else
                op.newContent = val;
        }
        ...
        {
            size_t j = i;
            while (j < s.size() && (s[j] == '-' || s[j] == '+' || s[j] == '.' || isdigit((unsigned char)s[j])))
                ++j;
            string num = s.substr(i, j - i);
            if (key == "timestamp")
                op.timestamp = atoll(num.c_str());
            else if (key == "lineNumber")
                op.lineNumber = atoi(num.c_str());
            else if (key == "startCol")
                op.startCol = atoi(num.c_str());
            else if (key == "endCol")
                op.endCol = atoi(num.c_str());
            i = j;
        }
        skip();
        if (i < s.size() && s[i] == ',')
                                   ...
                                   const vector<string> &new_lines)
{
    ...
    for (size_t ln = 0; ln < max_len; ++ln)
    {
        ...
        size_t start = 0;
        while (start < oldL.size() && start < newL.size() && oldL[start] == newL[start])
            ++start;

        size_t oldTail = 0, newTail = 0;
        while (oldTail < oldL.size() - start && newTail < newL.size() - start &&
               oldL[oldL.size() - 1 - oldTail] == newL[newL.size() - 1 - newTail])
        {
            ++oldTail;
            ++newTail;
        }

        size_t oldEnd = oldL.size() - oldTail;
        size_t newEnd = newL.size() - newTail;

        string removed = (oldEnd >= start) ? oldL.substr(start, oldEnd - start) : "";
        string added = (newEnd >= start) ? newL.substr(start, newEnd - start) : "";

        string opType = "REPLACE";
        if (removed.empty() && !added.empty())
            opType = "INSERT";
        else if (!removed.empty() && added.empty())
            opType = "DELETE";

        CRDTOperation op{opType, user_id, now_ms(), static_cast<int>(ln),
                         static_cast<int>(start),
                         static_cast<int>(start + (int)removed.size()),
                         removed, added};
...
void send_update(const string &queue_name, const string &json)
{
    ...
    if (mq == (mqd_t)-1)
        return; // dest not up yet
    if (mq_send(mq, json.c_str(), min((size_t)MAX_MSG_SIZE, json.size() + 1), 0) == -1)
    {
...
// --------------- Remote Update Broadcast Feature ---------------
void broadcast_remote_updates(const string &self, const vector<CRDTOperation> &remote_updates)
{
    if (remote_updates.empty())
        return;

    auto reg = load_registry();
    for (const auto &op : remote_updates)
    {
        string payload = op_to_json(op);
        for (const auto &[uid, ui] : reg)
        {
            // Don't send to self, original sender, or if we've already forwarded this
            if (uid == self || uid == op.userId)
                continue;
            if (!ui.queueName.empty() && ui.queueName[0] == '/')
                send_update(ui.queueName, payload);
        }
    ...
    // Show broadcast in terminal
    cout << "\n[ REMOTE UPDATE BROADCAST - Forwarding " << remote_updates.size() << " operations ]\n";
    for (auto &op : remote_updates)
    {
        cout << "  [FORWARD] " << op.type
             << " L" << op.lineNumber
             << " from " << op.userId
...
// --------------- CRDT (LWW) helpers ---------------
static inline bool overlaps(const CRDTOperation &a, const CRDTOperation &b)
{
    if (a.lineNumber != b.lineNumber)
        return false;
    return !(a.endCol <= b.startCol || b.endCol <= a.startCol);
}
static inline bool lww_wins(const CRDTOperation &a, const CRDTOperation &b)
{
    if (a.timestamp != b.timestamp)
        return a.timestamp > b.timestamp;
    return a.userId < b.userId; // tie-break: lexicographically smaller wins
}
static void apply_one_op_to_line(string &line, const CRDTOperation &op)
{
    if (!op.oldContent.empty())
    {
        size_t pos = line.find(op.oldContent);
        if (pos != string::npos)
        {
            line.replace(pos, op.oldContent.size(), op.newContent);
            return;
        }
    }
    int s = max(0, min(op.startCol, (int)line.size()));
    int e = max(0, min(op.endCol, (int)line.size()));
    if (e < s)
        e = s;
    string prefix = line.substr(0, s);
    string suffix = line.substr(e);
    line = prefix + op.newContent + suffix;
}
static void merge_LWW_apply_remote(vector<string> &lines,
                                   const vector<CRDTOperation> &local_ops_since,
                                   vector<CRDTOperation> &remote_ops)
{
    if (remote_ops.empty())
        return;
    vector<CRDTOperation> winners;
    winners.reserve(remote_ops.size());

    for (const auto &rop : remote_ops)
    {
        bool blocked = false;
        for (const auto &lop : local_ops_since)
        {
            if (overlaps(rop, lop) && !lww_wins(rop, lop))
            {
                blocked = true;
                break;
            }
        ...
        if (!blocked)
            winners.push_back(rop);
    }

    map<int, vector<CRDTOperation>> per_line;
    for (auto &op : winners)
        per_line[op.lineNumber].push_back(op);

    for (auto &[ln, ops] : per_line)
    {
        if (ln >= (int)lines.size())
            lines.resize(ln + 1);
        sort(ops.begin(), ops.end(),
             [](const CRDTOperation &a, const CRDTOperation &b)
             {
        ...
        for (auto &op : ops)
            apply_one_op_to_line(lines[ln], op);
    }
    remote_ops.clear(); // consumed
}
                      ...
                      size_t outbuf_size)
{
    ...
    char tbuf[32];
    strftime(tbuf, sizeof(tbuf), "%H:%M:%S", localtime(&raw));
...
void listener_thread(const string &user_id)
{
    ...
    while (running.load(memory_order_relaxed))
    {
        ...
        if (n >= 0)
        {
            ...
            // SPSC push; if full, drop oldest semantics: pop one then push
            if (!incoming_spsc.push(op))
            {
                CRDTOperation tmp;
                incoming_spsc.pop(tmp); // free a slot
...
int main(int argc, char *argv[])
{
    ...
    // Buffers owned by main thread only (no locks needed)
    vector<CRDTOperation> outgoing_updates_buffer; // local ops pending broadcast
    vector<CRDTOperation> local_since_merge;       // local ops since last merge
    vector<CRDTOperation> pending_remote;          // drained from SPSC; awaiting merge
    vector<CRDTOperation> remote_for_broadcast;    // remote ops to be broadcasted

    long long last_merge_ms = now_ms();
    long long last_remote_broadcast_ms = now_ms();
    ...
    while (running.load(memory_order_relaxed))
    {
        try
        {
            ...
            // --- Broadcast local updates after threshold
            if (outgoing_updates_buffer.size() >= BROADCAST_BATCH_SIZE)
            {
                ...
                // ---- SHOW LOCAL BROADCAST IN TERMINAL ----
                cout << "\n[ LOCAL BROADCAST - Sent " << BROADCAST_BATCH_SIZE << " operations ]\n";
                for (auto &op : to_send)
                {
                    cout << "  [SENT] " << op.type
                         << " L" << op.lineNumber
            ...
            // --- Drain listener SPSC into pending_remote (no locks)
            {
                CRDTOperation got;
                size_t drained = 0;
                while (incoming_spsc.pop(got))
                {
                    // ignore our own echoes if any (defensive)
                    if (got.userId != user_id)
                    {
                        pending_remote.push_back(got);
                        remote_for_broadcast.push_back(got); // Add to broadcast buffer
                    }
                    ++drained;
                    if (drained > 2048)
                        break; // safety cap per cycle
                }
            }

            // --- Broadcast remote updates after threshold
            if (remote_for_broadcast.size() >= REMOTE_BROADCAST_THRESHOLD)
            {
                // Take a fixed batch of remote operations for broadcasting
                vector<CRDTOperation> to_broadcast(
                    remote_for_broadcast.begin(),
                    remote_for_broadcast.begin() + REMOTE_BROADCAST_THRESHOLD);

                // Erase the batch from the buffer
                remote_for_broadcast.erase(
                    remote_for_broadcast.begin(),
                    remote_for_broadcast.begin() + REMOTE_BROADCAST_THRESHOLD);

                // Broadcast these remote updates to other users
                broadcast_remote_updates(user_id, to_broadcast);
                last_remote_broadcast_ms = now_ms(); // Update timestamp
            }

            // --- Merge trigger: threshold OR time-based if anything pending
            bool need_merge = false;
            if (local_since_merge.size() + pending_remote.size() >= MERGE_THRESHOLD)
                need_merge = true;
            ...
            if (need_merge)
            {
                ...
                // Apply remote winners; never re-apply local (already in file)
                vector<CRDTOperation> remote_for_merge = pending_remote; // local copy
                merge_LWW_apply_remote(new_lines, local_since_merge, remote_for_merge);

                // Write back & update state
                write_document_content(user_doc_path, new_lines);
                last_content = new_lines;
                last_mtime = get_file_mtime(user_doc_path);

                // Clear consumed remote buffer and local window
                pending_remote.clear();
                local_since_merge.clear();
                last_merge_ms = now_ms();
            }

            // --- Update registry lastSeen
            {
                auto reg = load_registry();
            ...
            // Show a snapshot of pending_remote (non-destructive)
            display_terminal(user_id, user_doc_path, last_content, active_users,
                             cycle_updates, pending_remote, outgoing_updates_buffer.size());
        }
        catch (const exception &e)
        {
